...
| Zefix (CH, nowe firmy) | ⏸️ zaimplementowane, czeka na dane logowania | Wymaga HTTP Basic Auth (schemat `Zefix-Credentials`, potwierdzone w [Swagger UI](https://www.zefix.admin.ch/ZefixPublicREST/swagger-ui/index.html)). Dostęp podobno przez mail na **zefix@bj.admin.ch** — to nieoficjalnie potwierdzone (cytat z README wrapperów community, nie ze strony oficjalnej), warto zweryfikować przy pisaniu. Dwuetapowe pobieranie: `/sogc/bydate/{date}` (nowe wpisy) → `/company/ehraid/{id}` (pełne dane: `purpose` do filtrowania branżowego, `cantonalExcerptWeb` = link do pełnego wyciągu kantonalnego, czyli ta sama zakładka "cantonal excerpt" widoczna na stronie Zefix). Potem ustaw `ZEFIX_USER`/`ZEFIX_PASS`. |
